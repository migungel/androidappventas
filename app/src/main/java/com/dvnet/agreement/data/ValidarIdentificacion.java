...
public class ValidarIdentificacion {
    ...
    /**
     * @param numero ruc institucion pulica
     * @return
     * @throws Exception
     */
    public boolean validarRucSociedadPublica(String numero) throws Exception {
        try {
            ...
            validarCodigoEstablecimiento(numero.substring(10, 13));
            algoritmoModulo11(numero.substring(0, 9), Integer.parseInt(String.valueOf(numero.charAt(9))), TipoDocumento.getRucPublica());
        } catch (Exception e) {
            return false;
        }
        ...
        protected boolean algoritmoModulo11(String digitosIniciales, int digitoVerificador, Integer tipo) throws Exception {
            ...
            List<Integer> digitosInicialesTMP = new ArrayList<>(digitosIniciales.length());
            int bound = digitosIniciales.length();
            for (int i = 0; i < bound; i++) {
                Integer integer = NumberUtils.createInteger(String.valueOf(digitosIniciales.charAt(i)));
                digitosInicialesTMP.add(integer);
            }
            /*
            ...
            AtomicInteger consolidadodMultiplicacionIndiceConeficiente = new AtomicInteger();
            List<Integer> finalArrayCoeficientes = arrayCoeficientes;
            int bound1 = arrayCoeficientes.size();
            for (int x = 0; x < bound1; x++) {
                int i = (digitosInicialesTMP.get(x) * finalArrayCoeficientes.get(x));
                consolidadodMultiplicacionIndiceConeficiente.addAndGet(i);
            }
